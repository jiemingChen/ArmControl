...
vector<Eigen::Affine3d> Panda::multiFrames(){
    // Create the frame that will contain the results
    KDL::Frame cartpos;
    Eigen::Affine3d O_EE;
    vector<Eigen::Affine3d> multilinks;

    KDL::Chain chain;
    std::string tip_link;
    vector<string> tip_vec = {};
    for(auto i=0; i<5; i++){
        tree_.getChain(base_link_, tip_vec[i], chain);
