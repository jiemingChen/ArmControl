...
private:
    bool initialized;
    ros::NodeHandle nh;
    ros::Publisher publisherCollisionModell, traj_pub_;
...
public:
    CollisionChecker() : initialized(false), octomapCollisionObject(nullptr), isFirstReceive(true) {
        if (!initialize()){
            ros::shutdown();
            return;
        }
        ...
        octomap::OcTree octTree(0.05);
        octTree.readBinary(octomapName);
        setOcTree(&octTree);

        publisherCollisionModell = nh.advertise<visualization_msgs::MarkerArray>("collision_modell", 3);  //-1
        sensorSub = nh.subscribe("/robot1/joint_states", 1, &CollisionChecker::jointStatesCallback, this);
...
//msg.joint_names.push_back("test");
//point_msg.time_from_start.fromSec(ros::Time::now().toSec());
//point_msg.transforms.resize(1);
//
//point_msg.transforms[0].translation.x= link->transform.p[0];
//point_msg.transforms[0].translation.y= link->transform.p[1];
//point_msg.transforms[0].translation.z= link->transform.p[2];
//
//point_msg.transforms[0].rotation.x = quatX;
//point_msg.transforms[0].rotation.y = quatY;
//point_msg.transforms[0].rotation.z = quatZ;
//point_msg.transforms[0].rotation.w = quatW;
//if(link->name== "panda_link2"){  //|| link->name== "panda_link1"
//cout << link->joint->getName() << link->joint->JointAxis()[0]  <<" " <<  link->joint->JointAxis()[1] <<" "<< link->joint->JointAxis()[2]   << endl;
////                cout << link->joint->pose(*(link->jointValue)).p[0] << " " << link->joint->pose(*(link->jointValue)).p[1] << " " << link->joint->pose(*(link->jointValue)).p[2] << endl;
//auto test2 = link->joint->pose(*(link->jointValue)).M;
//auto test3 = kdlTree.getSegment("panda_link2")->second.segment.getJoint().pose(jointPositions[1]).M;
//auto test = kdlTree.getSegment("panda_link2")->second.segment.pose(jointPositions[1]).M;
//cout << test(0,0) << " " << test(0,1) << " " << test(0,2) << "\n" <<
//test(1,0) << " " << test(1,1) << " " << test(1,2) << "\n" <<
//test(2,0) << " " <<test(2,1) << " " << test(2,2) << endl;
//cout << "----------------" << endl;
//cout << test2(0,0) << " " << test2(0,1) << " " << test2(0,2) << "\n" <<
//test2(1,0) << " " << test2(1,1) << " " << test2(1,2) << "\n" <<
//test2(2,0) << " " << test2(2,1) << " " << test2(2,2) << endl;
//cout << "----------------" << endl;
//cout << test3(0,0) << " " << test3(0,1) << " " << test3(0,2) << "\n" <<
//test3(1,0) << " " << test3(1,1) << " " << test3(1,2) << "\n" <<
//test3(2,0) << " " << test3(2,1) << " " << test3(2,2) << endl;
//cout << (*(link->jointValue)== jointPositions[1]) << endl;
//
////                cout <<  (*(link->transformParent)).p[0] << " " <<  (*(link->transformParent)).p[1]<< " " <<  (*(link->transformParent)).p[2] << endl;
////                cout <<  ((link->transform)).p[0] << " " << ((link->transform)).p[1]<< " " << ((link->transform)).p[2] << endl;
//
//cout << link->joint->pose(*(link->jointValue)).p[0] << " " << link->joint->pose(*(link->jointValue)).p[1] << " " << link->joint->pose(*(link->jointValue)).p[2] << endl;
